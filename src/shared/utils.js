...
export function replaceOrAppendNestedEntries({obj, replacements, append_mode = false}) {
    for (const [key, value] of Object.entries(replacements)) {
      if (typeof value === 'object' && value !== null && !(value instanceof Array)) {
        // If the replacement value is an object, recurse into the corresponding nested object
        if (key in obj && typeof obj[key] === 'object' && obj[key] !== null) {
          replaceOrAppendNestedEntries(obj[key], value);
        } else {
          obj[key] = value; // If the key doesn't exist, add the replacement object
        }
      } else {
